...
int copynFile(FILE * origin, FILE * destination, int nBytes){
	return fwrite(destination, 1, nBytes, origin);
}
...
stHeaderEntry* readHeader(FILE * tarFile, int *nFiles){
	...
	for(i = 0; i < nr_files; i++){
		...
		if(fread(headerArray[i].name, sizeof(headerArray[i].name), 1, tarFile) < 1){
			printf("Error en la lectura del nombre del archivo %d cabecera del fichero. \n", i);
			return NULL;
		}
